...
const { deletePost, toggleLove } = actions;
...
class Post extends React.Component {
    ...
    onOption(){
        ...
        ActionSheetIOS.showActionSheetWithOptions({
            ...
            (buttonIndex) => {
                if (buttonIndex === 0) Actions.NewPost({ edit:true, post })
    ...
    onDelete(){
        ...
        this.props.deletePost(post, (error) =>  alert(error.message))
    }
    ...
    render() {
        const { user, posts, index } = this.props;
        const post = posts[index];
        const { text, author, time, color, userId } = post;
...
export default connect(mapStateToProps, { deletePost, toggleLove })(Post);